...
import { Animated, View,ImageBackground ,TouchableOpacity,Image, StyleSheet, PanResponder} from "react-native";
 ...
 const   Joystick = (props) =>{
  const {actIndex,actionSteeringWheel}= props
  const [move,setMoving] = useState(false)
  const [goIng,setDown] = useState(null)
  const [max,setMax] = useState(0)
  const [pan, setPan] = useState({ x: 0, y: 0 });
   var  start // 记录第一次的更新的pan
    const panResponder = useRef(
      PanResponder.create({       
        ...
        onPanResponderMove: (event, gestureState) => {
          ...
          if((start >0 && pos.y<0 )||(start < 0 && pos.y>0)){
            pos.y = 0
          }
