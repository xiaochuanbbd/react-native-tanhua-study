...
const Demo = () => {
  ...
  const rotateBoxep = useRef(null);
  let rotateBoxXY = { pageX: 0, pageY: 0 };
  let rotateBoxepXY = { pageX: 0, pageY: 0 };
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      const rotatepxRef = await layoutRef(rotateBoxep.current);
      //计算第一个块的固定坐标点（不会变动）
      ...
      rotateBoxepXY.pageX = rotatepxRef.pageX + rotatepxRef.width / 2;
      rotateBoxepXY.pageY = rotatepxRef.pageY + rotatepxRef.height / 2;
    });
  }, []);
